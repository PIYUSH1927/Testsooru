...
const ExportsPanel: React.FC<ExportsPanelProps> = ({ onSelectOption }) => {
    ...
    const { floorPlanData, updateVisualizationOption, visualizationOptions } = useFloorPlan();
    ...
    const exportAsPNG = async () => {
        ...
        try {
            const clonedParentContainer = parentContainer.cloneNode(true) as HTMLElement;
            const clonedContainer = clonedParentContainer.querySelector('.floor-plan-container') as HTMLElement;
            ...
            if (svg) {
                ...
                clonedContainer.style.height = (bbox.height + exportPadding + topPadding) + 'px';

                svg.setAttribute('viewBox', `${bbox.x - exportPadding} ${bbox.y - topPadding} ${bbox.width + 2 * exportPadding} ${bbox.height + exportPadding + topPadding}`);
            }

            const originalAreaText = parentContainer.querySelector('.always-black-text') as HTMLElement;

            if (originalAreaText && includeMeasurements) {
                const textContent = originalAreaText.innerText || originalAreaText.textContent || '';
            ...
            const dimensionLabels = clonedContainer.querySelectorAll('.dimension-label');
            dimensionLabels.forEach(label => {
                (label as HTMLElement).style.display = includeMeasurements ? 'block' : 'none';
            });
            ...
            roomLabels.forEach(label => {
                ...
                } else if ((label as HTMLElement).classList.contains('dimension-label')) {
            ...
            saveAs(dataUrl, `floor-plan-${new Date().toISOString().split('T')[0]}.png`);
        } catch (error) {
    ...
    const exportAsPDF = async () => {
        ...
        try {
            ...
            // Original area text processing - similar to the PNG export
            const originalAreaText = parentContainer.querySelector('.always-black-text') as HTMLElement;
            ...
            await new Promise((resolve) => {
                img.onload = () => {
                    ...
                    // Remove the additional text at the bottom
                    // No text additions here anymore
    ...
    const exportAsCAD = async () => {
        try {
            ...
            const bounds = floorPlanData.rooms.reduce((acc, room) => {
            ...
            dxfContent += '30\n0\n';
            dxfContent += '40\n0.5\n';
            dxfContent += `1\nTotal Area: ${floorPlanData.total_area.toFixed(2)} m² | Total Rooms: ${floorPlanData.room_count}\n`;
            dxfContent += '50\n0\n';
            dxfContent += '72\n1\n';
            dxfContent += '73\n2\n';
    ...
    return (
        <div className="panel-options exports-options" style={{
            userSelect: "none",
            ...
            <div className="checkbox-control" style={{position:"relative", left:"0.3rem"}}>
                <input
                    type="checkbox"
